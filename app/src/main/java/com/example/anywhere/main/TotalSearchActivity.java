...
//        recyclerView4 = findViewById(R.id.accommodationsRecycler);
    ...
    private void runthread(String cIdType,RecyclerView recycler) {
        ...
        new Thread(() -> {
            ...
            runOnUiThread(() -> {
                switch (cIdType){
                    case "12":  mAdapter = new ListAdapter(tourList,getApplicationContext());   break;
                    case "39":  mAdapter = new ListAdapter(foodList,getApplicationContext());   break;
                    case "15":  mAdapter = new ListAdapter(festivalList,getApplicationContext());   break;
                    case "32":  mAdapter = new ListAdapter(AccomList,getApplicationContext());   break;
                }
                ...
                if(count==0){
                    switch (cIdType){
                        case "12":   trip_non_text.setVisibility(View.VISIBLE); break;
                        case "39":   food_non_text.setVisibility(View.VISIBLE);  break;
                        case "15":   festival_non_text.setVisibility(View.VISIBLE);  break;
                        case "32":   accom_non_text.setVisibility(View.VISIBLE);  break;
                    }
                }
            });
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                if(imgurl != null&&title!= null&&addr!= null&&id!= null){
                    switch (cid){
                        case "12":  tourList.add(new ItemList(title,imgurl,addr,id));   break;
                        case "39":  foodList.add(new ItemList(title,imgurl,addr,id));   break;
                        case "15":  festivalList.add(new ItemList(title,imgurl,addr,id));   break;
                        case "32":  AccomList.add(new ItemList(title,imgurl,addr,id));   break;
                    }
